...
final class ParserExpressionTests {
    ...
    private static Stream<Arguments> testExpressionStatement() {
        return Stream.of(
                ...
                Arguments.of("Missing Semicolon",
                        ...
                        null // expect ParseException: Expected ';'.
                )
        );
    }
    ...
    private static Stream<Arguments> testAssignmentStatement() {
        return Stream.of(
                ...
                Arguments.of("Missing Value",
                        ...
                        null // expect ParseException: Expected expression.
                )
        );
    }
    ...
    private static Stream<Arguments> testAccessExpression() {
        return Stream.of(
                ...
                Arguments.of("Invalid Name After Dot",
                        Arrays.asList(
                                // obj.5   (should be identifier, not integer)
                                new Token(Token.Type.IDENTIFIER, "obj", 0),
                                new Token(Token.Type.OPERATOR, ".", 3),
                                new Token(Token.Type.INTEGER, "5", 4)
                        ),
                        null // expect ParseException: Expected identifier.
                )
        );
    }
    ...
    @ParameterizedTest
    @MethodSource
    void testExceptions(String test, List<Token> tokens, Object ignored) {
        // these should all throw
        Assertions.assertThrows(ParseException.class, () -> {
            // choose the entry point based on what the test looks like
            if (test.contains("Statement")) {
    ...
    private static Stream<Arguments> testExceptions() {
        return Stream.of(
                Arguments.of("Invalid Expression", Arrays.asList(
                        // ?
                        new Token(Token.Type.OPERATOR, "?", 0)
                ), null),
                Arguments.of("Missing Closing Parenthesis (expr)", Arrays.asList(
                        // (expr
                        new Token(Token.Type.OPERATOR, "(", 0),
                        new Token(Token.Type.IDENTIFIER, "expr", 1)
                ), null),
                Arguments.of("Invalid Closing Parenthesis", Arrays.asList(
                        // (expr]
                        new Token(Token.Type.OPERATOR, "(", 0),
                        new Token(Token.Type.IDENTIFIER, "expr", 1),
                        new Token(Token.Type.OPERATOR, "]", 5)
                ), null),
                Arguments.of("Statement Missing Semicolon", Arrays.asList(
                        // name()
                        new Token(Token.Type.IDENTIFIER, "name", 0),
                        new Token(Token.Type.OPERATOR, "(", 4),
                        new Token(Token.Type.OPERATOR, ")", 5)
